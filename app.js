...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function animateChildren(section) {
        ...
        if (section.classList.contains('about')) {
            children = section.querySelectorAll('.about-intro, .about-image-container, .about-text, .values-section, .approach-section, .team-section');
        } else if (section.classList.contains('services')) {
    ...
    sections.forEach(section => {
        if (!section.classList.contains('hero')) {
            const children = section.querySelectorAll('.about-intro, .about-image-container, .about-text, .values-section, .approach-section, .team-section, .service-card, .process-step, .portfolio-item, .testimonial, .why_us-header, .comparison-table, .contact-info, .contact-form');
    ...
    const floatingElements = document.querySelectorAll('.experience-badge, .image-accent');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleScrollAnimations() {
        ...
        sectionHeaders.forEach(header => {
            if (isElementInViewport(header, 0.2) && !header.classList.contains('animated')) {
        ...
        valueCards.forEach(card => {
            if (isElementInViewport(card, 0.1) && !card.classList.contains('animated')) {
        ...
        approachSteps.forEach(step => {
            if (isElementInViewport(step, 0.1) && !step.classList.contains('animated')) {
        ...
        statNumbers.forEach(stat => {
            if (isElementInViewport(stat, 0.2) && !stat.classList.contains('animated')) {
